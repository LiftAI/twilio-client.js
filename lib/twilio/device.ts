...
class Device extends EventEmitter {
  ...
  async updateOptions(options: Device.Options = { }): Promise<void> {
    if (this._calls.length > 0 || this.activeCall) {
      this._log.warn('Existing Device has ongoing calls; ignoring new options.');
      return;
    }
   ...
   * For example, our call:accept is not triggered if the handler for device:connect handler throws an error.
   * As a side effect, we are not able to perform our internal routines such as stopping incoming sounds.
   * See call:accept inside _makeCall where we call emit('connect'). This can throw an error.
   * See call:accept inside _onSignalingInvite. This handler won't get called if the error above is thrown.
   * @private
   */
  ...
  private async _makeCall(twimlParams: Record<string, string>, options?: Call.Options): Promise<Call> {
    ...
    options = Object.assign({
      ...
      beforeAccept: (newCall: Call) => {
        if (!this._activeCall || this._activeCall === newCall) {
          return;
        }
    ...
    call.once('disconnect', () => {
      ...
      this._removeCall(call);
      this._asyncEmit('disconnect', call);
    });
  ...
  private _onSignalingOffline = () => {
    ...
    this._edge = null;
    this._region = null;
    this.emit('offline', this); // TODO Should this be different than unregister???
  }
...
namespace Device {
  /**
   * Emitted when an incoming {@link Call} is canceled.
   * @param call - The canceled {@link Call}.
   * @example `device.on('cancel', call => { })`
   * @event
   */
  declare function cancelEvent(call: Call): void;

  /**
   * Emitted when a {@link Call} has been opened.
   * @param call - The {@link Call} that was opened.
   * @example `device.on('connect', call => { })`
   * @event
   */
  declare function connectEvent(call: Call): void;

  /**
   * Emitted when a {@link Call} has been disconnected.
   * @param call - The {@link Call} that was disconnected.
   * @example `device.on('disconnect', call => { })`
   * @event
   */
  ...
  export enum EventName {
    ...
    Error = 'error',
    Incoming = 'incoming',
    Offline = 'offline', // TODO do we need this now?
    Ready = 'ready', // TODO do we need this now?
    Unregistered = 'unregistered', // TODO how is this different than offline?
